...
def init_pygame():
    ...
    try:
        ...
        pygame.init()
        display = (800, 600)
        pygame.display.set_caption("Domain Flow Matmul Visualization")
...
def render_frame(spacetime_events, current_time, nodes):
    """Render the 3D scene for the current time step."""
    try:
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        glLoadIdentity()
        gluLookAt(1.5, 1.5, 1.5, 0.5, 0.5, 0.5, 0, 1, 0)  # Center on grid
        ...
        for node_id, events in spacetime_events.items():
            if nodes[node_id]["operator"] == "matmul":
                for x, y, z, t in events:
                    if abs(t - current_time) < 0.2:
                        # Simulate A[i,k] at (x, 0, z) (i, k plane)
...
async def main():
    ...
    try:
        ...
        while current_time <= max_time + 5:
            ...
            render_frame(spacetime_events, current_time, node_dict)
            ...
            if frame_count % FPS == 0:
                print(f"Frame {frame_count}, time {current_time:.2f}")
    except Exception as e:
